...
def numerical_relativity_postmerger_waveform(newtime, **waveform_kwargs):
    ...
    snr=22
    loudness = 2.5e-21 * snr / 50
    
    t_0=0
    
    # location of the NR waveform
    #location =r'/home/valentina.dimarco/postm/postmerger_tbilby/post_merger_tBilby/BAM_0101-master/R01/'
    location =r'/home/valentina.dimarco/postm/postmerger_tbilby/post_merger_tBilby/R01/'
...
def inner_product(aa, bb, freq, psd, model_variance):
    ...
    arguments:
    aa, bb: single-sided Fourier transform, created, e.g., by the nfft function
...
def damped_sin(x,t0,A,tau,f_Hz,alpha,phi_rad):  
...
def test_model(get_fake_data=False):
    ...
    model = damped_sin(t,A=10**(-22),tau=0.005,f_Hz=2000,phi_rad=0*np.pi/180,alpha=2)
    plt.close('all')
...
#(n_damped_sin,'A','tau','f_Hz','phi_rad','alpha')
#Init_params={}
...
#Init_params['n_damped_sin']=3   
#Init_params['t0']=0.00 


componant_functions_dict={}
componant_functions_dict[damped_sin]=(n_damped_sin,'A','tau','f_Hz','phi_rad','alpha')
...
#priors_t ['n_damped_sin'] = tbilby.core.prior.DiscreteUniform(1,n_damped_sin,'n_damped_sin')
#priors_t = tbilby.core.base.create_transdimensional_priors(TransdimensionalConditionalUniform_f,'f_Hz',nmax=n_damped_sin,nested_conditional_transdimensional_params=['f_Hz'],conditional_params=[],prior_dict_to_add=priors_t,minimum=1000,maximum=5000)
#priors_t  = tbilby.create_plain_priors(Uniform,'f_Hz',n_damped_sin,prior_dict_to_add=priors_t,minimum=1000,maximum=5000)
#priors_t = tbilby.core.base.create_transdimensional_priors(TransdimensionalConditionalLogUniform_A,'A',nmax=n_damped_sin,nested_conditional_transdimensional_params=['A'],conditional_params=[],prior_dict_to_add=priors_t,minimum=10**(-23),maximum=10**(-17))


#priors_t  = tbilby.create_plain_priors(Uniform,'A',n_damped_sin,prior_dict_to_add=priors_t,minimum=10**(-24), maximum=10**(-19))
#priors_t  = tbilby.core.base.create_plain_priors(Uniform,'tau',n_damped_sin,prior_dict_to_add=priors_t,minimum=10**(-5), maximum=0.1)
#priors_t  = tbilby.core.base.create_plain_priors(Uniform,'phi_rad',n_damped_sin,prior_dict_to_add=priors_t,minimum=-np.pi , maximum=np.pi )
#priors_t  = tbilby.core.base.create_plain_priors(Uniform,'alpha',n_damped_sin,prior_dict_to_add=priors_t,minimum=-6.4 , maximum=6.4 )

priors_t = bilby.core.prior.dict.ConditionalPriorDict()
#n,f,A,tau,phi priors
priors_t ['n_damped_sin'] = tbilby.core.prior.DiscreteUniform(1,n_damped_sin,'n_damped_sin')
#priors_t ['n_damped_sin'] = 3
#priors_t = tbilby.create_transdimensional_priors(TransdimensionalConditionalUniform_f,'f_Hz',nmax=n_damped_sin,nested_conditional_transdimensional_params=['f_Hz'],conditional_params=[],prior_dict_to_add=priors_t,minimum=1000,maximum=5000)
#priors_t = tbilby.create_transdimensional_priors(TransdimensionalConditionalLogUniform_A,'A',nmax=n_damped_sin,nested_conditional_transdimensional_params=['A'],conditional_params=[],prior_dict_to_add=priors_t,minimum=10**(-23),maximum=10**(-17))
priors_t  = tbilby.core.base.create_plain_priors(LogUniform,'A',n_damped_sin,prior_dict_to_add=priors_t,minimum=10**(-24), maximum=10**(-19))
priors_t  = tbilby.core.base.create_plain_priors(Uniform,'f_Hz',n_damped_sin,prior_dict_to_add=priors_t,minimum=1000, maximum=5000)
priors_t  = tbilby.core.base.create_plain_priors(LogUniform,'tau',n_damped_sin,prior_dict_to_add=priors_t,minimum=5*10**(-4), maximum=5*10**(-2))
priors_t  = tbilby.core.base.create_plain_priors(Uniform,'phi_rad',n_damped_sin,prior_dict_to_add=priors_t,minimum=-np.pi, maximum=np.pi)
priors_t  = tbilby.core.base.create_plain_priors(Uniform,'alpha',n_damped_sin,prior_dict_to_add=priors_t,minimum=-6.4 , maximum=6.4 )
...
fname = r'/home/valentina.dimarco/postm/postmerger_tbilby/post_merger_tBilby/sensitivity_curves/A_sharp.txt'
aLIGO_O3 = np.genfromtxt(fname)
freq=aLIGO_O3[:,0]
asd = aLIGO_O3[:,1]

NoiseBudgetFile = r'/home/valentina.dimarco/postm/postmerger_tbilby/post_merger_tBilby/sensitivity_curves/OzHF_NoiseBudget_AllTraces.hdf5'
ff = h5py.File(NoiseBudgetFile, 'r')
list(ff.keys())
print('HERE')
for k in ff.keys():
    print(k)

frequency_domain_strain_NEMO = np.abs(ff['Freq'])

# Set up NEMO as a four-kilometer detector in Gingin
#NEMO = bilby.gw.detector.Interferometer(
#    power_spectral_density=bilby.gw.detector.PowerSpectralDensity(
#        frequency_array=ff.freq, psd_array=ff.psd
#    ),
#    name="NEMO",
#    length=4,
...
outdir='outdir_no_ord_4'
label='no_ord_4_psd'
ifos.plot_data(outdir=outdir, label=label)

# here we can fix the some parameters and let the others vary

#const=['A', 'tau','phi_rad','alpha']
const=[]
for c in const:
    for i in np.arange(n_damped_sin):
...
if not plot_result:
    ...
    df = result.posterior
    plt.figure()
    plt.hist(result.posterior['n_damped_sin'])
    ...
    plt.figure()
    # plotting the damped sinusoids
    t=np.arange(0,duration,1/sampling_frequency)
    #model_parameters['tau1']=0.0002
    #model_parameters['A1']=7*10**(-20)
    #model_parameters['A0']=2*10**(-20)
    for n in np.arange(int(model_parameters['n_damped_sin'])):
        plt.subplot(3,4,n+1)
        #plt.plot(t,damped_sin(t,model_parameters['A'+str(n)],model_parameters['tau'+str(n)],model_parameters['f_Hz'+str(n)],model_parameters['alpha'+str(n)], model_parameters['phi_rad'+str(n)])['cross'])
    plt.subplot(3,4,12)
    plt.savefig('sinusoids_no_ordering_4.png')
    
    plt.figure()
    # subplot in time domain
    plt.subplot(2,1,1)
    plt.plot(t,model(t,**model_parameters)['cross'],'-r',label='Fit',alpha=0.6)
    plt.plot(t,NRmodel(t,t_0=0)['cross'],'b',alpha=0.6,label='NR')
    plt.legend()
         
    # subplot in freq domain    
    plt.subplot(2,1,2)
    ...
    plt.legend()
    plt.savefig('fit_no_ordering_4.png')
    plt.figure()

    tbilby.core.base.corner_plot_discrete_params(result, 'number_of_damped_sins_no_ord_4.png')
